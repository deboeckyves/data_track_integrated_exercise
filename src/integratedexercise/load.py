...
def main():
    ...
    print(df_ppm_all_stations.count())
    df_ppm_all_stations.show()
    df_ppm_all_stations.show(n=5, truncate=False, vertical=True)

    print(get_snowflake_creds_from_sm("snowflake/integrated-exercise/yves-login"))
    sfOptions = {'sfURL': 'yw41113.eu-west-1.snowflakecomputing.com', 'sfPassword': 'ilovedbt', 'sfUser': 'axxes_yves', 'sfDatabase': 'ACADEMY_DBT', 'sfWarehouse': 'COMPUTE_WH', 'sfSchema': 'AXXES_YVES', 'sfRole': 'STUDENT'}
    # read full table
    df = spark.read.format("snowflake").options(**sfOptions).option("dbtable", "SNOWFLAKE_CASES_TRANS").load()

    df_ppm_all_stations.printSchema()
